...
typedef MetricTapCallback =
    void Function(BuildContext context, DashboardMetric metric);
...
class DashboardOverview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final activeOrders = dummyActiveOrders.length;
    final revenuePipeline = dummyActiveOrders.fold<double>(
      0,
      (total, order) => total + order.amount,
    );
    final avgTicket = activeOrders == 0 ? 0.0 : revenuePipeline / activeOrders;
    final recentHistory = dummyOrderHistory.take(4).toList();
    final expenseTotals = _categoryTotals();
    final expenseSum = dummyExpenses.fold<double>(
      0,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text('${dummyExpenses.length} logged expenses'),
                  const SizedBox(height: 12),
                  Wrap(
                    spacing: 8,
                    runSpacing: 8,
