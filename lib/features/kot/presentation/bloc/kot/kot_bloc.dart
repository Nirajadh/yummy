...
class KotBloc extends Bloc<KotEvent, KotState> {
  ...
  Future<void> _onRequested(KotRequested event, Emitter<KotState> emit) async {
    emit(state.copyWith(status: KotStatus.loading));
    try {
      final tickets = await _getKotTicketsUseCase();
      emit(state.copyWith(status: KotStatus.success, tickets: tickets));
    } catch (e) {
      emit(
        state.copyWith(status: KotStatus.failure, errorMessage: e.toString()),
      );
    }
  }
}
