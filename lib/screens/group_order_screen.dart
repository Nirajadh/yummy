...
import 'group_order/person_order_sheet.dart';

// Backward compatibility for hot-reload/lookups referencing old private classes.
typedef _OrderLine = GroupOrderLine;
typedef _PaymentEntry = GroupPaymentEntry;
...
class _GroupOrderScreenState extends State<GroupOrderScreen> {
  ...
  void _removeLine(int index) {
    if (index < 0 || index >= _orderLines.length) return;
    final line = _orderLines[index];
    if (line.isHistoric) {
      _showMessage('Previous orders cannot be edited here.');
      return;
    }
    setState(() {
    ...
    _showMessage('${line.label} removed.');
  }

  void _openLineDetail(int index) {
    if (index < 0 || index >= _orderLines.length) return;
    final line = _orderLines[index];
    if (line.isHistoric) {
      _showMessage('Previous orders cannot be edited here.');
    ...
    showPersonOrderSheet(
      context: context,
      line: line,
      onSave: (updated) {
        setState(() {
          _orderLines[index] = updated;
          _recalcTotals();
        });
      },
    );
  ...
  void _editLine(int index) {
    ...
    final itemsController = TextEditingController(
      text: (line.items ?? const []).isEmpty
          ? ''
          : (line.items ?? const []).join(', '),
    );
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  ...
                  hintText: 'e.g., Pizza x2, Salad x1',
                ),
              ),
              ...
              FilledButton(
                onPressed: () {
                  ...
                  if (addNewPerson) {
                    ...
                    setState(() {
                      _people.add(GroupPerson(name: assignedTo));
                    });
                  }
                ...
                FilledButton(
                  onPressed: () {
                    ...
                    if (addNewPerson) {
                      ...
                      setState(() {
                        _people.add(GroupPerson(name: targetName ?? ''));
                      });
                    }
